...
export async function getCaseById(sessionId, caseId) {
  return getItem(`SESSION#${sessionId}`, `CASE#${caseId}`);
}
...
export async function updateReminder(sessionId, reminderId, patch) {
  const existing = await getItem(`SESSION#${sessionId}`, `REMINDER#${reminderId}`);
...
export async function deleteReminder(sessionId, reminderId) {
  return deleteItem(`SESSION#${sessionId}`, `REMINDER#${reminderId}`);
}
